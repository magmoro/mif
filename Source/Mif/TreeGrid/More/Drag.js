...
Mif.TreeGrid.Drag = new Class({
	...
	initialize: function(tree, options){
		...
		this.current=Mif.TreeGrid.Drag.current;
		this.target=Mif.TreeGrid.Drag.target;
		this.where=Mif.TreeGrid.Drag.where;
		...
		this.addEvent('start', function(){
			Mif.TreeGrid.Drag.current=this.current;
			Mif.TreeGrid.Drag.startZone=this;
			Mif.TreeGrid.Drag.dropZone=this;
			this.tree.unselect();
			...
			Mif.TreeGrid.Drag.current.getDOM('wrapper').addClass('mif-tree-draggable');
			this.addGhost();
		}, true);
		this.addEvent('complete', function(){
			...
			Mif.TreeGrid.Drag.current.getDOM('wrapper').removeClass('mif-tree-draggable');
			var dropZone=Mif.TreeGrid.Drag.dropZone;
			if(!dropZone || dropZone.where=='notAllowed'){
				Mif.TreeGrid.Drag.startZone.onstop();
				Mif.TreeGrid.Drag.startZone.emptydrop();
				return;
			}
		...
		Mif.TreeGrid.Drag.current=false;
	},
	...
	addToGroups: function(groups){
		...
		groups.each(function(group){
			Mif.TreeGrid.Drag.groups[group]=(Mif.TreeGrid.Drag.groups[group]||[]).include(this);
		}, this);
	},
	...
	setDroppables: function(droppables){
		...
		this.groups.each(function(group){
			this.droppables.combine(Mif.TreeGrid.Drag.groups[group]);
		}, this);
	},
	...
	leave: function(event){
		...
		if(dropZone){
			...
			Mif.TreeGrid.Drag.ghostIcon.className='mif-tree-ghost-icon mif-tree-ghost-'+dropZone.where;
			if(dropZone.onleave) dropZone.onleave();
			Mif.TreeGrid.Drag.dropZone=false;
		}
	...
	enter: function(event, zone){
		...
		Mif.TreeGrid.Drag.dropZone=zone;
		zone.current=Mif.TreeGrid.Drag.current;
	...
	keydown: function(event){
		if(event.key=='esc') {
			var zone=Mif.TreeGrid.Drag.dropZone;
	...
	autoScroll: function(){
		...
		var top=y-container.scrollTop;
		var bottom=container.clientHeight-top;
		var sign=0;
		...
		if(sign && !this.scrolling){
			this.scrolling=function(node){
				if(y!=this.y){
					y=this.y;
					delta = (sign==1 ? (y-container.scrollTop) : (container.clientHeight-y+container.scrollTop))||1;
				}
				container.scrollTop=container.scrollTop-sign*this.options.scrollSpeed/delta;
	...
	start: function(event){//mousedown
		if(Mif.TreeGrid.Drag.current) return;
	...
	drag: function(event){
		Mif.TreeGrid.Drag.ghost.position({x:event.page.x+20,y:event.page.y+20});
		var dropZone=Mif.TreeGrid.Drag.dropZone;
		if(!dropZone||!dropZone.ondrag) return;
		Mif.TreeGrid.Drag.dropZone.ondrag(event);
	},
	...
	ondrag: function(event){
		...
		Mif.TreeGrid.Drag.ghostIcon.className='mif-tree-ghost-icon mif-tree-ghost-'+ghostType;
		if(where == 'notAllowed'){
			this.tree.unselect();
			return;
		}
	...
	addGhost: function(){
		...
		ghost.adopt(Mif.TreeGrid.Draw.node(this.current).getElement('.mif-tree-light'))
		.injectInside(document.body).setStyle('position', 'absolute');
		Mif.TreeGrid.Drag.ghostIcon=new Element('span', {'class': 'mif-tree-ghost-icon mif-tree-ghost-notAllowed'}).inject(ghost.getElement('.mif-tree-left'), 'after');
		Mif.TreeGrid.Drag.ghost=ghost;
	},
	...
	emptydrop: function(){
		...
		var complete=function(){
			...
			if(this.options.animate){
				...
				Mif.TreeGrid.Drag.ghost.set('morph',{
					duration: 'short',
					onComplete: function(){
				...
				Mif.TreeGrid.Drag.ghost.morph({left: position.x, top: position.y});
				return;
			};
			Mif.TreeGrid.Drag.ghost.dispose();
			this.fireEvent('emptydrop', this.element);

...
Mif.TreeGrid.Node.implement({
	...
	inject: function(node, where, domNode){//domNode - internal property
		...
		Mif.TreeGrid.Draw.updateDOM(this, domNode);
		[node, this, parent, previous, this.getPrevious()].each(function(node){
			Mif.TreeGrid.Draw.update(node);
		});
		tree.$getIndex();
	...
	copy: function(node, where){
		function copy(structure){
			...
			var property=$unlink(node.property);
			//property.open=false;
			var nodeCopy = new Mif.TreeGrid.Node({
		...
		return nodeCopy.inject(node, where, Mif.TreeGrid.Draw.node(nodeCopy));
	},
	...
	remove: function(){
		...
		Mif.TreeGrid.Draw.update(parent);
		Mif.TreeGrid.Draw.update(previous);
...
Mif.TreeGrid.implement({
	...
	add: function(node, current, where){
		if($type(node)!='mif:tree:node'){
			node=new Mif.TreeGrid.Node({
				parentNode: null,
				tree: this
			}, node);
		};
		node.inject(current, where, Mif.TreeGrid.Draw.node(node));
